...
- Agent discovery may read host MCP config files for **server names only**; it never persists env, args, headers, URLs, or login tokens.
...
- Writing nodes must reach an independent `verifier` node; acceptance criteria and checks stay frozen unless a new graph version is amended and re-approved.
