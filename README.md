...
[![Cursor](https://img.shields.io/badge/Cursor-Skills_+_MCP-000000?style=for-the-badge)](https://cursor.com/docs/skills)
[![Agent Skills](https://img.shields.io/badge/Agent_Skills-Portable-0EA5E9?style=for-the-badge)](https://agentskills.io)
...
![OpenGraph Dashboard](apps/web/public/og.png)

OpenGraph packages a deterministic graph compiler, an approval-gated runtime, and a live Dashboard into one installable **Graph** plugin for coding agents. The portable core is an Agent Skill plus a planner-safe MCP server — installable for Codex, Claude Code, Cursor, Qoder, ZCode, OpenClaw, Hermes, Kimi, Gemini CLI, Qwen Code, and any host that loads `~/.agents/skills`. The host model plans; deterministic code validates; you remain the approval boundary.
...
Loop Engineering solves automation (execute → measure → retry). Graph Engineering adds supervision so automation does not Goodhart itself: independent verifier nodes watch writers, acceptance criteria stay frozen, and humans remain the value anchor. OpenGraph compiles both into one reviewable graph.
...
| Writers self-score and may weaken tests    | Independent supervisor paths + frozen acceptance              |
...
### Local installer (recommended — all agents)
...
```bash
./install-graph cursor    # also: codex claude qoder zcode openclaw hermes kimi gemini qwen agents
```

Then start a **new** agent session and invoke Graph with a goal:

```text
$graph <goal>           # Codex
/graph <goal>           # Claude Code, Cursor, Qoder, ZCode, …
/skill:graph <goal>     # Kimi
```
...
`./install-graph claude` also installs the shorter `/graph` skill alias.

Requirements: **Node.js 20+** and **Git**. The first invocation installs the local SQLite runtime dependency. See the [planner host matrix](docs/agent-adapters.md) for per-agent skill and MCP paths.
...
- **Review** — inspect nodes by supervision role (execute / supervise / accept / anchor), dependencies, prompts, agents, read/write scopes, retry budgets, timeouts, inputs, outputs, and frozen acceptance checks.
- **Run** — follow normalized activity, tool calls, terminal output, diffs, tests, artifacts, retries, check failures vs supervision rejections, and reconnect-safe event replay.
...
- Code-writing nodes require fresh, read-only verification and a path to an independent `verifier` node.
- Acceptance criteria stay frozen for workers; changing them needs an amended, re-approved graph version.
...
```text
Any planner host (Codex, Claude, Cursor, Qoder, ZCode, OpenClaw, Hermes, …)
       │  $graph / /graph / skill:graph
       ▼
...
See the [development guide](docs/development.md) for source boundaries, quality checks, and plugin builds.
...
OpenGraph is an early, working local-first implementation. The plugin, compiler, runtime, Dashboard, persistence layer, recovery path, test suite, and multi-host installer (Codex, Claude Code, Cursor, Qoder, ZCode, OpenClaw, Hermes, Kimi, Gemini, Qwen, shared Agent Skills) are implemented. Production ACP worker adapters beyond the included mock/demo path remain an active area of development.
